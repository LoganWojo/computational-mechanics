...
```{code-cell} ipython3
def Future_prediction(years,USA_data,EUR_data,Predictive_year):
    '''Take in a year range and the Energy values for USA and Europe at those years and use liner regression 
        to predict a value at a future year'''
    Year_value=Predictive_year
    mUSA2, bUSA2 = np.polyfit(years,USA_data, 1)
    mEUR2, bEUR2 = np.polyfit(years,EUR_data, 1)

    USA_prediction= mUSA2*Year_value + bUSA2
    EUR_prediction= mEUR2*Year_value + bEUR2

    print('The predictive value of Energy use for the USA in the year {:.0f} is {:.1f} TW-hrs'.format(Year_value,USA_prediction))
    print('The predictive value of Energy use for Europe in the year {:.0f} is {:.1f} TW-hrs'.format(Year_value,EUR_prediction))

Future_prediction(x2,yUSA2,yEUR2,2025)
```
...
```{code-cell} ipython3
print('The value of the constant A is {:.2f} transistors/year'.format(a))
print('The value of the constant B is {:.2f} transistors'.format(b))
```

```{code-cell} ipython3
date=np.arange(1970,2020,1)

TC_1=np.zeros(len(date))

for i in range(len(date)):
    TC_1[i]=np.exp(b)*np.exp(a*date[i])

Moores_test_1=np.zeros(len(TC_1)-1)

for j in range(len(Moores_test_1)):
    Moores_test_1[j]=(TC_1[j+1]-TC_1[j])/TC_1[j]

Avg_percent_Moore1=np.mean(Moores_test_1)*100*2

print('The average percent increase in number of transistors every 2 years was {:.2f}%'.format(Avg_percent_Moore1))
```

Based on our linear regression analysis we find that the average percent increase in the number of MOS transistors every 2 years is roughly 80%. Moore's law predicted that the number of transistors would double every 2 years, wwhile our linear regression model predicts that they will double every 2.5 years. Given that there was very limited data for Moore to make his prediction, he produced a model which actually became fairly accurate all factors considered.
